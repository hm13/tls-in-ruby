...
# p digest("key", "message-to-be-authenticated")

def hkdf_extract(salt, ikm)
  hmac_hash_binary(salt, ikm)
end
...
def hkdf_expand(prk, info, l)
  ...
  t = [""]
  (0..n-1).each do |i|
    t << hmac_hash_binary(prk, t[i] + info << i+1)
  end

  t.join.bytes.take(l).pack("C*")
end
